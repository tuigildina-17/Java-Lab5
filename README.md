...
Интерфейс FractionInterface - задает контракт для работы с дробями
Методы интерфейса:
double getValue() — возвращает вещественное значение дроби
void setNumerator(int n) — устанавливает числитель
void setDenominator(int d) — устанавливает знаменатель
Класс Fraction:
Поля класса:
int numerator — числитель
int denominator — знаменатель
Double cachedValue — кэш вычисленного значения дроби
Методы класса:
Конструктор Fraction(int numerator, int denominator) — создаёт дробь
setNumerator(int n) — меняет числитель и сбрасывает кэш
setDenominator(int d) — проверяет знаменатель, запрещает 0, переносит знак
getValue() — вычисляет значение дроби, кэширует результат
...
2.Создаётся объект Fraction.
3.Программа выводит дробь и её значение.
4.При повторном вызове getValue() используется кэш.
5.Пользователь меняет числитель — кэш сбрасывается.
6.Программа сравнивает две дроби методом equals().
...
int meowCount — счётчик мяуканий
Методы класса:
Конструктор Cat(String name) — создаёт кота
void meow() — выводит "Имя: мяу!" и увеличивает счётчик
toString() — "кот: Имя"
Интерфейс Meowable:
Метод интерфейса: void meow() — объект умеет мяукать
Метод Funs.meowsCare(Meowable m) - Принимает объект, который умеет мяукать, и вызывает его meow() несколько раз

Алгоритм работы:
1.Пользователь вводит имя кота.
2.Создаётся объект Cat.
3.Кот передаётся в метод meowsCare.
4.Метод вызывает meow() несколько раз.
5.После завершения работы выводится количество мяуканий.
...
Класс Point:
Поля класса: x, y
Методы класса: toString(), equals(), hashCode()
Класс Polyline:
Поля класса: List<Point> points  
Методы класса: toString()
Класс StreamTask:
