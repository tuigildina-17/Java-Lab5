...
void meow() — реализация интерфейса Meowable, выводит "Имя: мяу!"
toString() — "кот: Имя"
Интерфейс Meowable:
Метод интерфейса: void meow() — объект умеет мяукать
Класс Funs:
Метод static int meowsCare(Meowable m) - Вызывает метод meow() у переданного объекта 5 раз и возвращает количество вызовов
