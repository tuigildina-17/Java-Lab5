...
Класс MeowCounterWrapper - класс, реализующий структурный шаблон проектирования «Декоратор»
Поля класса:
Meowable original - ссылка на оригинальный объект (например, Cat)
int count - внутренний счётчик мяуканий
Методы класса:
Конструктор MeowCounterWrapper(Meowable original) - принимает объект, который нужно «обернуть» декоратором
void meow() - перехватывает вызов: вызывает original.meow() и увеличивает счётчик count++
int getCount() - возвращает количество мяуканий, зафиксированных декоратором
toString() - делегирует вывод оригинальному объекту
Класс MeowCounter - вызывает meow() у всех Meowable
Метод класса:
static void meowsCare(Meowable m, int n) - принимает объект Meowable и вызывает его метод meow() n раз

Алгоритм работы:
1.Пользователь вводит имя кота.
2.Создаётся объект Cat.
3.Кот оборачивается в декоратор MeowCounterWrapper, который будет считать мяуканья.
4.Объект-декоратор передаётся в метод MeowCounter.meowsCare(), который вызывает meow() несколько раз (например, 5).
5.Каждый вызов:
- вызывает meow() у оригинального кота,
- увеличивает внутренний счётчик декоратора.
6.После завершения работы выводится:
- имя кота (через toString()),
- количество мяуканий, зафиксированных декоратором (getCount())
